...
impl ModelManager {
    ...
    async fn ensure_loaded(&mut self, vault_path: &str) -> Result<(), String> {
        ...
        let tokenizer = Tokenizer::from_file(tokenizer_path.to_str().unwrap_or_default())
            .map_err(|e| e.to_string())?;
    ...
    async fn embed(&mut self, text: &str) -> Result<Vec<f32>, String> {
        ...
        for i in 0..seq_len {
            for j in 0..EMBEDDING_DIM {
                mean_embedding[j] += raw_data[i * EMBEDDING_DIM + j];
            }
        }
        for j in 0..EMBEDDING_DIM {
...
async fn search_hnsw(
    ...
    // If index file doesn't exist, build it first (async reads, then sync index build).
    let hnsw_exists = tokio::fs::metadata(&hnsw_path).await.is_ok();
    if !hnsw_exists {
        build_index_async(&hnsw_path, manifest, base_dir).await?;
    }

    // All async work is done above. Now do sync-only index operations (Index is !Send).
...
async fn build_index_async(
    ...
    // Read all embeddings first (async), then build the index synchronously
    // because usearch::Index is !Send and cannot be held across .await points.
