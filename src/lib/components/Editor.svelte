  ...
  const commandList = [
    {
      id: "drawing",
      label: "Drawing",
      insert: async () => await insertDrawing(),
    },
    {
      id: "math",
      label: "Math Block",
      insert: async () => insertBlock("$$\n\n$$"),
    },
    {
      id: "table",
      label: "Table",
    ...
    { id: "file", label: "File", insert: async () => await insertFile() },
    {
      id: "callout",
      label: "Callout",
      insert: async () => insertBlock("> [!note] Title\n> Details\n"),
    },
    {
      id: "toc",
      label: "Table of Contents",
      insert: async () => insertBlock("[[toc]]\n"),
    },
    {
      id: "reminder",
      label: "Reminder",
      insert: async () => insertBlock("- [ ] Reminder: \n"),
    },
  ];
  ...
  class AssetWidget extends WidgetType {
    ...
    toDOM(): HTMLElement {
      ...
      if (lower.endsWith(".json")) {
        ...
        if (currentVault) {
          tauriInvoke<string>("load_drawing", {
      ...
      if (lower.endsWith(".pdf")) {
        ...
        let scale = 0.9;
        if (currentVault) {
          tauriInvoke<string>("resolve_asset", {
            vaultPath: currentVault,
            filename: this.filename,
          })
            .then((path) => {
          ...
          while ((match = regex.exec(text))) {
            ...
            if (token.startsWith("!")) {
              ...
              builder.add(
                ...
                Decoration.widget({
                  widget: new AssetWidget(filename),
                  side: 1,
                }),
              );
            }
        ...
        for (const range of view.visibleRanges) {
          ...
          while ((match = blockRegex.exec(text))) {
            ...
            builder.add(
              ...
              Decoration.widget({ widget: new MathWidget(match[1], true) }),
            );
          }
          while ((match = inlineRegex.exec(text))) {
            ...
            builder.add(
              ...
              Decoration.widget({ widget: new MathWidget(match[1], false) }),
            );
          }
        }
        return builder.finish();
        ...
        while (lineNumber <= doc.lines) {
          ...
          if (trimmed.startsWith("```")) {
            ...
            builder.add(
              ...
              line.from,
              Decoration.widget({
                widget: new CodeBlockHeader(lang, code),
                side: -1,
              }),
            );
        ...
        for (let i = 1; i <= doc.lines; i++) {
          ...
          if (trimmed.startsWith("> [!")) {
            ...
            builder.add(
              line.from,
              line.from,
              Decoration.line({ class: `callout callout-${type}` }),
            );
            continue;
          }
          if (activeType && trimmed.startsWith(">")) {
            builder.add(
              line.from,
              line.from,
              Decoration.line({ class: `callout callout-${activeType}` }),
            );
          } else {
            activeType = null;
          }
...
<div class="editor-shell">
  ...
  {#if commandOpen}
    <div
      ...
      style={`top:${commandY}px; left:${commandX}px;`}
    >
